...
void region_file_writer::write(void) {
	...
	for (unsigned int i = 0; i < region_dim::CHUNK_COUNT; ++i) {
		...
		remaining = region_dim::SECTOR_SIZE - (reg.get_header().get_info_at(i).get_length() % region_dim::SECTOR_SIZE) -
