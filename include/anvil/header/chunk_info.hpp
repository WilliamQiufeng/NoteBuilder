...
public:
	 ...
	 * Chunk info constructor
	 */
	chunk_info(const chunk_info &other) : modified(other.modified), offset(other.offset), length(other.length),
										  type(other.type) { return; }

	/*
	 * Chunk info constructor
	 */
	chunk_info(unsigned int offset, unsigned int length, char type, unsigned int modified) : modified(modified),
																							 offset(offset),
																							 length(length),
																							 type(type) { return; }
